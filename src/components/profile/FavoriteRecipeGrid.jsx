// src/components/profile/FavoriteRecipeGrid.jsx
import { Clock, Star, ChefHat, Heart } from 'lucide-react';
import { useState, useEffect, useRef } from 'react';
import FavoriteButton from '../common/FavoriteButton';

export default function FavoriteRecipeGrid({ recipes, onRecipeClick }) {
  const [visibleCards, setVisibleCards] = useState(new Set());
  const cardRefs = useRef([]);

  useEffect(() => {
    // Pastikan refs di-reset saat daftar resep berubah
  ...
  // Tampilan jika tidak ada resep favorit
  if (recipes.length === 0) {
    return (
      <div className="text-center py-16 bg-white/60 backdrop-blur-sm rounded-3xl p-6 md:p-8 shadow-xl border border-white/40">
        <Heart className="w-16 h-16 text-slate-300 mx-auto mb-4" />
        <h3 className="text-xl font-semibold text-slate-700">Resep Favorit Kosong</h3>
        <p className="text-slate-500 mt-2">Anda belum menambahkan resep apapun ke favorit.</p>
        <p className="text-slate-400 text-sm mt-1">Klik ikon ❤️ pada resep untuk menyimpannya di sini.</p>
      </div>
    );
  }

  // Tampilan grid jika ada resep favorit
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
      {recipes.map((recipe, index) => (
        ...
        >
          <div 
            // Kirim 'category' agar halaman detail tahu warna apa yang harus ditampilkan
            ...
            <div className="relative z-10 p-4 md:p-8">
              <div className="flex items-center justify-between mb-3 md:mb-4">
                ...
                <span className={`text-xs font-semibold capitalize px-2 md:px-3 py-1 md:py-1.5 rounded-full ${
